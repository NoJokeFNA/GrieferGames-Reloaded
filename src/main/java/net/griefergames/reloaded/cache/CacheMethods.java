...
public class CacheMethods {
  ...
  private static void clearCache() {
    for (val cacheUuid : CacheHandler.USER_MAP.keySet()) {
      ...
      val player =
          GrieferGamesReloaded.PLUGIN.getPlugin().getServer().getPlayer(user.getPlayerUuid());
      if (player == null) CacheHandler.USER_MAP.remove(user.getPlayerUuid());

      GrieferGamesLogger.log(
          Level.INFO, "Current cached players: {0}", new Object[] {CacheHandler.USER_MAP.size()});
    }
  }
}
