 ...
 *
 * <ul>
 *   <li>fill the {@code inventory} with the {@link org.bukkit.inventory.ItemStack} of your choice;
 *       you can also fill it with different colors
 *   <li>set an {@link org.bukkit.inventory.ItemStack} delayed; you can choose if you wanna play a
 *       {@link org.bukkit.Sound}, or not
 * </ul>
 */
public class InventoryBuilder {
   ...
   *
   * @param size Set the {@code size} of the inventory you've created You can only use numbers that
   *     are divisible by nine The maximum is fifty-four and the minimum is nine
   * @param displayName Enter the {@code displayName} of the inventory you created to be displayed
   *     above
   */
  ...
  public InventoryBuilder(
      @NotNull final Player player, @NotNull final InventoryType inventoryType) {
  ...
  public InventoryBuilder fillInventory(@NotNull final ItemStack itemStack) {
    for (short i = 0; i < this.inventory.getSize(); i++) this.setItem(i, itemStack);
    return this;
  }

  /**
   * Fill the {@code inventory} with {@code itemContents}
   *
   ...
   *
   * @param itemStack Create the {@code itemStack} you want to fill up the inventory
   * @param maximalRandom Set the {@code maximalRandom} value, to be used for the {@code subId} of
   *     the current {@code itemStack}
   */
  public InventoryBuilder fillInventoryRandomColors(
      @NotNull final ItemStack itemStack, final int maximalRandom) {
   ...
   *
   * @param itemStack Create the {@code itemStack} you want to fill up the inventory
   * @param minimalRandom Set the {@code minimalRandom} value, from the start value, of the {@code
   *     subId}
   * @param maximalRandom Set the {@code maximalRandom} value, to be used for the {@code subId} of
   *     the current {@code itemStack}
   */
  public InventoryBuilder fillInventoryRandomColors(
      @NotNull final ItemStack itemStack, final int minimalRandom, final int maximalRandom) {
    for (short index = 0; index < this.inventory.getSize(); index++) {
      val damageMeta = (Damageable) itemStack.getItemMeta();
      assert damageMeta != null;
      damageMeta.setDamage(
          (short) ThreadLocalRandom.current().nextInt(minimalRandom, maximalRandom));
  ...
  /**
   * Fill the {@code inventory} with the {@code itemStack} you selected, from {@code from} to {@code
   * to}
   *
   * @param itemStack Create the {@code itemStack} you want to fill up the inventory
   * @param from Set the {@code from} value, where we should start to calc
   * @param to Set the {@code to} value, to be used for the {@code subId} of the current {@code
   *     itemStack}
   */
  public InventoryBuilder fillInventoryFromTo(
      @NotNull final ItemStack itemStack, final short from, final short to) {
    for (short i = from; i < to; i++) this.setItem(i, itemStack);
    return this;
  }
   ...
   */
  public InventoryBuilder setDelayedItem(
      final int slot,
